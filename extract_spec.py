...
from test_esm import  FastaBatchedDataset, esm_1b_emb, pretrained
...
def main(args):
    net, alphabet = pretrained.load_model_and_alphabet(args.model_location)
    ...
    model.embed_tokens.load_state_dict(net.embed_tokens.state_dict(), strict=True)
    model.eval()
    ...
    with torch.no_grad():
        #for batch_idx,(toks, labels, strs) in enumerate(data_loader):
